...
organization bearer routes; enrollment-token issuance remains a `201` with its public response schema.
Canonical operator paths use `/api/v1/**`; unversioned aliases do not exist. Canonical
...
Canonical responses are asserted against their executable schemas. A separately invoked built-server PostgreSQL release
proof covers project listing, configuration validation/install, manual dispatch, and daemon enrollment with colliding slugs and delivery keys in two
...
The self-hosted Scalar page at `/api/reference` renders only the public CLI login and operator operation groups;
