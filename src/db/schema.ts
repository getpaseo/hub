...
export const providerEventRoutingDecisions = pgTable(
  ...
  (table) => [
    ...
    check(
      ...
      sql`${table.code} in ('no_trigger_for_source', 'connection_mismatch', 'resource_mismatch', 'repository_mismatch', 'guild_mismatch', 'workspace_mismatch', 'channel_mismatch', 'sender_not_allowed', 'contains_mismatch', 'pattern_mismatch', 'input_filter_mismatch', 'invocation_rejected', 'configuration_unavailable', 'no_project_route', 'routing_evidence_truncated')`,
    ),
    check(
...
export const providerEventRoutingOutcomes = pgTable(
  "provider_event_routing_outcomes",
  {
    id: uuid().defaultRandom().primaryKey(),
    organizationId: text("organization_id").notNull(),
    providerEventReceiptId: uuid("provider_event_receipt_id").notNull(),
    status: text().$type<"pending" | "routed" | "dropped">().notNull(),
    expectedProjectCount: integer("expected_project_count").notNull(),
  ...
  (table) => [
    uniqueIndex("provider_event_routing_outcomes_receipt_unique").on(table.providerEventReceiptId),
    index("provider_event_routing_outcomes_status_idx").on(
      table.organizationId,
      table.status,
      table.createdAt,
    ),
    check(
      "provider_event_routing_outcomes_status_check",
      sql`${table.status} in ('pending', 'routed', 'dropped')`,
    ),
    check(
      "provider_event_routing_outcomes_counts_check",
      sql`${table.expectedProjectCount} >= 0
        and ${table.completedProjectCount} >= 0
        and ${table.completedProjectCount} <= ${table.expectedProjectCount}
        and ${table.routedProjectCount} >= 0
        and ${table.routedProjectCount} <= ${table.completedProjectCount}
        and ((${table.status} = 'pending' and ${table.finalizedAt} is null)
          or (${table.status} in ('routed', 'dropped') and ${table.finalizedAt} is not null))`,
    ),
    foreignKey({
      columns: [table.organizationId],
      foreignColumns: [organizations.id],
      name: "provider_event_routing_outcomes_organization_fk",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.providerEventReceiptId, table.organizationId],
      foreignColumns: [providerEventReceipts.id, providerEventReceipts.organizationId],
      name: "provider_event_routing_outcomes_receipt_organization_fk",
    }).onDelete("cascade"),
  ],
);

export const providerEventRoutingProjectResults = pgTable(
  "provider_event_routing_project_results",
  {
    id: uuid().defaultRandom().primaryKey(),
    organizationId: text("organization_id").notNull(),
    providerEventReceiptId: uuid("provider_event_receipt_id").notNull(),
    projectId: uuid("project_id").notNull(),
    configurationRevisionId: uuid("configuration_revision_id").notNull(),
    status: text().$type<"routed" | "dropped">().notNull(),
    createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  },
  (table) => [
    uniqueIndex("provider_event_routing_project_results_receipt_project_unique").on(
      table.providerEventReceiptId,
      table.projectId,
    ),
    index("provider_event_routing_project_results_receipt_idx").on(
      table.providerEventReceiptId,
      table.createdAt,
    ),
    check(
      "provider_event_routing_project_results_status_check",
      sql`${table.status} in ('routed', 'dropped')`,
    ),
    foreignKey({
      columns: [table.organizationId],
      foreignColumns: [organizations.id],
      name: "provider_event_routing_project_results_organization_fk",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.providerEventReceiptId, table.organizationId],
      foreignColumns: [providerEventReceipts.id, providerEventReceipts.organizationId],
      name: "provider_event_routing_project_results_receipt_organization_fk",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.projectId, table.organizationId],
      foreignColumns: [projects.id, projects.organizationId],
      name: "provider_event_routing_project_results_project_organization_fk",
    }).onDelete("cascade"),
    foreignKey({
      columns: [table.configurationRevisionId, table.projectId, table.organizationId],
      foreignColumns: [
      ...
      name: "provider_event_routing_project_results_revision_project_organization_fk",
    }).onDelete("cascade"),
  ],
);
