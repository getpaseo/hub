...
class PgDatabase implements Database {
  ...
  constructor(private readonly pool: Pool) {
    ...
    this.workflows = new WorkflowRepository(pool, {
      query,
      insertAgentExecution: insertAgentExecutionOnClient,
      reserveOrganizationUsage: reserveOrganizationUsageOnClient,
      findAgentExecutionById: (id) => this.findAgentExecutionById(id),
      transitionAgentExecution: (id, status, fields) =>
        this.transitionAgentExecution(id, status, fields),
    });
  }
  ...
  createAcceptedTriggerRun(input: CreateAcceptedTriggerRunInput) {
    return this.workflows.createAcceptedTriggerRun(input);
  }

  createRejectedTriggerRun(input: CreateRejectedTriggerRunInput) {
    return this.workflows.createRejectedTriggerRun(input);
  }

  findTriggerRunById(id: string) {
    return this.workflows.findTriggerRunById(id);
  }

  findTriggerRunsByProviderEventReceiptId(providerEventReceiptId: string) {
    return this.workflows.findTriggerRunsByProviderEventReceiptId(providerEventReceiptId);
  }

  listTriggerRunsForProject(projectId: string, limit: number) {
    return this.workflows.listTriggerRunsForProject(projectId, limit);
  }

  findWorkflowStepRunById(id: string) {
    return this.workflows.findWorkflowStepRunById(id);
  }

  findWorkflowStepRunByTriggerRun(triggerRunId: string) {
    return this.workflows.findWorkflowStepRunByTriggerRun(triggerRunId);
  }

  listWorkflowStepRunsForTriggerRun(triggerRunId: string) {
    return this.workflows.listWorkflowStepRunsForTriggerRun(triggerRunId);
  }

  findAgentExecutionByWorkflowStepRunId(stepRunId: string) {
    return this.workflows.findAgentExecutionByWorkflowStepRunId(stepRunId);
  }

  claimWorkflowWakeup(now: Date, leaseMs: number) {
    return this.workflows.claimWorkflowWakeup(now, leaseMs);
  }

  wakeWorkflowRun(triggerRunId: string, availableAt: Date) {
    return this.workflows.wakeWorkflowRun(triggerRunId, availableAt);
  }

  deleteWorkflowWakeup(triggerRunId: string) {
    return this.workflows.deleteWorkflowWakeup(triggerRunId);
  }

  createWorkflowStepExecution(input: WorkflowStepExecutionInput) {
    return this.workflows.createWorkflowStepExecution(input);
  }
  ...
  linkWorkflowStepRunExecution(
    ...
    return this.workflows.linkWorkflowStepRunExecution(stepRunId, executionId, dispatchIntent);
  }
  ...
  completeWorkflowStep(
    ...
    return this.workflows.completeWorkflowStep(executionId, status, result, failureReason);
  }

  completeWorkflowAgentExecution(input: WorkflowAgentCompletionInput) {
    return this.workflows.completeWorkflowAgentExecution(input);
  }

  markWorkflowStepSkipped(triggerRunId: string, stepId: string, reason: string) {
    return this.workflows.markWorkflowStepSkipped(triggerRunId, stepId, reason);
  }

  succeedTriggerRun(triggerRunId: string) {
    return this.workflows.succeedTriggerRun(triggerRunId);
  }
  ...
  failWorkflowRun(
    ...
    return this.workflows.failWorkflowRun(triggerRunId, status, failureReason, stepId);
  }

  recoverWorkflowWakeups(now: Date) {
    return this.workflows.recoverWorkflowWakeups(now);
  }

  claimPendingWorkflowRunTerminalNotification(now: Date, leaseMs: number) {
    return this.workflows.claimPendingWorkflowRunTerminalNotification(now, leaseMs);
  }

  markWorkflowRunTerminalNotificationDelivered(triggerRunId: string, deliveredAt: Date) {
    return this.workflows.markWorkflowRunTerminalNotificationDelivered(triggerRunId, deliveredAt);
  }

  recoverWorkflowDeadlines(now: Date) {
    return this.workflows.recoverWorkflowDeadlines(now);
  }
  ...
  async setAgentExecutionIdleDeadline(
    ...
    try {
      ...
      if (existing.workflow_step_run_id !== null) {
        workflowRefreshAllowed = await this.workflows.canRefreshStepIdleDeadline(
          client,
      ...
      if (row !== undefined && row.workflow_step_run_id !== null) {
        await this.workflows.updateStepIdleDeadline(
          client,
  ...
  updateTriggerRunValues(triggerRunId: string, values: unknown) {
    return this.workflows.updateTriggerRunValues(triggerRunId, values);
  }

  listProjectActivityRuns(projectId: string, limit: number) {
    return this.workflows.listProjectActivityRuns(projectId, limit);
  }

  findProjectActivityRun(projectId: string, runId: string) {
    return this.workflows.findProjectActivityRun(projectId, runId);
  }
